...
J += sum(sum((-1)*Y.*log(h) - (1-Y).*log(1-h)))
...
delta3 = h - Y	% 5000*10
delta2 = Theta2'*delta3'.*(a2.*(1-a2))'	% (26*10)x(10*1) -> 26*1
reg2 = [zeros(size(Theta2, 1), 1) Theta2(:, 2:end)]
reg1 = [zeros(size(Theta1, 1), 1) Theta1(:, 2:end)]
Theta2_grad = Theta2_grad + (1/m)*(delta3'*a2 + lambda*reg2)
Theta1_grad = Theta1_grad + (1/m)*(delta2(2:end, :)*X + lambda*reg1)	% 25x401
